...
// a chart containing example data to help display a graph when a user loads
...
async function getConfirmedCasesDailyData() {
    ...
    if (confirmedCasesData.length == 0) {
        ...
        confirmedCasesData = data.RWA.data;
    }
...
async function getConfirmedDeathsDailyData() {
    ...
    if (confirmedDeathData.length == 0) {
        const response = await fetch('../csv/ConfirmedDeaths.csv');
...
async function getVaccinatedPeopleDailyData() {
    ...
    rows.forEach(row => {
        ...
        if (columns[2] == "Rwanda") {
            ...
            const yVacinatedPeopleData = columns[40];
            yVaccinatedPeopleDailyData.push(yVacinatedPeopleData );
        }
...
async function getTravelRestrictions() {
    ...
    data.forEach(obj => {
        ...
        if (obj.adm0_name == "Rwanda") {
            result1 += "<strong>Published: </strong> " + obj.published + "<strong>Source: </strong> " + obj.sources + "<strong>Latest News:</strong> " + obj.info;
...
async function getAirlineRestritions() {
    ...
    data.forEach(obj => {
        ...
        if (obj.adm0_name == "Rwanda") {
            result += "<strong>Published:</strong> " + obj.published + "<strong>Source:</strong> " + obj.source + "<strong>Airline:</strong> "  + obj.airline + "<br>" + obj.info;
        }
